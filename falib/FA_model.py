...
t2_p = np.array([ 22.0, 44.0, 55.0, 46.0, 52.0, 52.0, 55.0, 55.0, 80.0 ]) * 1E-3
...
def gen_M(te,get_Mpinv=True,get_P0=False):
    ...
    M = tf.linalg.matmul(tf.math.exp(tf.linalg.matmul(2j*np.pi*te, f_p+1j*t2_p/(2*np.pi))), A_p) # shape: bs x ne x ns
...
def get_rho(acqs,param_maps,te,complex_data=False):
    ...
    voxel_shape = tf.convert_to_tensor((n_batch,hgt,wdt))
    num_voxel = tf.math.reduce_prod(voxel_shape)
    Smtx = tf.reshape(S, [n_batch, ne, num_voxel])
    ...
    rho_hat = tf.expand_dims(rho_hat,axis=-1)
