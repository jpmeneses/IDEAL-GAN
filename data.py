...
def load_hdf5(ds_dir,hdf5_file,ech_idx=12,start=0,end=2000,custom_list=None,num_slice_list=None,
    ...
    if MEBCRN:
        if mag_and_phase:
            ...
            out_w_pha = np.arctan2(out_maps[:,:,:,1:2],out_maps[:,:,:,:1])/np.pi
            out_f_pha = np.arctan2(out_maps[:,:,:,3:4],out_maps[:,:,:,2:3])/np.pi
            out_r2 = np.expand_dims(out_maps[:,:,:,4:5],axis=-1)
            out_fm = np.expand_dims(out_maps[:,:,:,5:],axis=-1)
            out_mag = np.concatenate((out_w_mag,out_f_mag,out_r2),axis=-1)
            out_pha = np.concatenate((out_w_pha,out_f_pha,out_fm),axis=-1)
