...
def load_hdf5(ds_dir,hdf5_file,ech_idx=12,start=0,end=2000,custom_list=None,num_slice_list=None,
    ...
    if MEBCRN:
        if mag_and_phase:
            ...
            out_w_pha = np.arctan2(out_maps[:,:,:,1:2],out_maps[:,:,:,:1])
            # out_w_pha = np.where(out_w_mag<=0.2,0.0,out_w_pha)
            ...
            out_wf_pha = np.expand_dims(unwrap_phase(np.squeeze(out_wf_pha)),axis=-1)
            out_mag = np.expand_dims(np.concatenate((out_w_mag,out_f_mag,out_maps[:,:,:,4:5]),axis=-1),axis=1)
            out_pha = np.expand_dims(np.concatenate((np.zeros_like(out_wf_pha),out_wf_pha/np.pi,out_maps[:,:,:,5:]),axis=-1),axis=1)
