...
def STDw_MSE(x,y,std,log_weight=0.1):
    ...
    if tf.reduce_sum(tf.cast(tf.math.is_nan(msd),tf.int32)) > 0:
        raise ValueError("Images MSE with NaNs")
    # std = tf.reduce_mean(std,axis=-1)
    std = tf.squeeze(std,axis=-1)
    # STDw_msd = tf.where(std!=0.0,msd/std,0.0)
    STDw_msd = tf.math.divide_no_nan(msd,std)
    if tf.reduce_sum(tf.cast(tf.math.is_nan(STDw_msd),tf.int32)) > 0:
        raise ValueError("NaNs generated by STD division")
    log_std = tf.where(std!=0.0,tf.math.log(std),0.0)
    if tf.reduce_sum(tf.cast(tf.math.is_nan(log_std),tf.int32)) > 0:
        raise ValueError("NaNs generated by log-STD")
    STDw_msd += log_std
