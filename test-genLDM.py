...
for k in range(args.n_samples):
    ...
    q_fig.colorbar(Fp_unet, ax=q_axs[0]).ax.tick_params(labelsize=14)
    q_axs[0].axis('off')
    r2_unet = q_axs[1].imshow(r2_aux*r2_sc, cmap='copper', vmin=0, vmax=r2_sc)
    q_fig.colorbar(r2_unet, ax=q_axs[1]).ax.tick_params(labelsize=14)
    q_axs[1].axis('off')
    field_unet = q_axs[2].imshow(field_aux*fm_sc, cmap='twilight', vmin=-fm_sc/2, vmax=fm_sc/2)
    q_fig.colorbar(field_unet, ax=q_axs[2]).ax.tick_params(labelsize=14)
    q_axs[2].axis('off')
    plt.subplots_adjust(top=1,bottom=0,right=1,left=0,hspace=0.1,wspace=0)
    ...
    pha_fig.colorbar(im_pha, ax=pha_ax).ax.tick_params(labelsize=14)
    pha_ax.axis('off')
    plt.subplots_adjust(top=1,bottom=0,right=1,left=0,hspace=0.1,wspace=0)
