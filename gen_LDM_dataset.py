...
for k in range(args.n_samples//args.batch_size):
    ...
    for i in range(Z2B.shape[0]):
        if args.save_dicom:
            ...
            ds2 = data.gen_ds(n_vol+i, method_prefix)
            data.write_dicom(ds2, image3d_2, path_2, filename_2, 0, np.shape(image3d_2)[2])
            ...
            for j in range(X3.shape[1]):
                ...
                data.write_dicom(ds3, image3d_3, path_3, filename_3, j, X3.shape[1])
        else:
            acqs_i = Z2B2A[i,...]
            out_maps_i = Z2B[i,...]
